...
KERNEL_X = 24
patience = 500                # if validation loss not going down, wait "patience" number of epochs
accuracy = 10
MaxCredit = -800
...
if OPTIMIZATION_PLUGIN == 'Bayesian' :
    ...
    def objective(SCI_RELU, SCI_BIAS, SCI_loss_type,
        ...
        validation_loader = Data.DataLoader(dataset = dataset.validation_dataset, batch_size = 123, shuffle = True, num_workers = 0, drop_last=True, pin_memory=True)    
        ...
        for epoch in range(SCI_EPOCHS):
            ...
            with torch.no_grad():
                ...
                if epoch % 2 == 0: 
                    SCI_LR, flag = Utillities.variable_learning_rate(SCI_LR, LR_MIN, LR_MAX, 1.5, flag)
                    optimizer = Utillities.optimization_algorithms(SCI_optimizer,cnn, SCI_LR, SCI_SGD_MOMENTUM,
                                                       SCI_REGULARIZATION)
    ...
    optimizer = BayesianOptimization(
        ...
        pbounds={'SCI_RELU': (1,1.99), 
                 ...
                 'SCI_LR': (0.05, 0.4), 
                 'SCI_MM': (0.001, 0.999), 
                 'SCI_REGULARIZATION': (0.01, 0.1), 
                 'SCI_EPOCHS': (2000, 2000), 
                 'SCI_BATCH_SIZE': (232, 232), 
                 'SCI_DROPOUT': (0.5, 0.85), 
                 'SCI_L_SECOND': (130, 169), 
